...
h=sec.header.paragraphs[0];h.add_run().add_picture(str(root/'assets/hietakulma-logo.png'),width=Cm(6.8));h.add_run('    LUONNOS 0.4  /  TARKISTETTAVAKSI').font.size=Pt(8)
...
def table(rows):
 ...
 for i,values in enumerate(data):
  ...
  for j,(cell,value) in enumerate(zip(row.cells,values)):
   ...
   p.paragraph_format.keep_with_next=((len(data)<=8 or data[0]==['Kenttä', 'Varaustieto']) and i<len(data)-1)
...
for index,path in enumerate(files):
 ...
 while i<len(lines):
  ...
  if line.startswith('Valittu verhoustyyppi'):p.paragraph_format.keep_with_next=True
...
d.core_properties.subject='Luonnos 0.4 Villen Jorman ja Tapanin tarkistettavaksi'
d.core_properties.author='Hietakulma'
out=root/'Hietakulma-tarkistuspaketti-LUONNOS-0.4.docx';d.save(out);print(out)
